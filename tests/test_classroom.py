...
@pytest.mark.parametrize(
    "student_name", ["Neville Longbottom", "Luna Lovegood", "Ginny Weasley"]
)
def test_add_student(harry_potter_classroom, student_name):
    student = Student(student_name)
    harry_potter_classroom.add_student(student)
    assert (
        student in harry_potter_classroom.students
    ), f"{student_name} should be in the classroom"
...
def test_remove_student(harry_potter_classroom):
    ...
    assert (
        "Ron Weasley" not in student_names
    ), "Ron Weasley should have been removed from the classroom"


def test_change_teacher(harry_potter_classroom, new_teacher):
    harry_potter_classroom.change_teacher(new_teacher)
    assert (
        harry_potter_classroom.teacher.name == "Minerva McGonagall"
    ), "Teacher should have been changed to Minerva McGonagall"
...
def test_remove_nonexistent_student(harry_potter_classroom):
    ...
    student_names = [student.name for student in harry_potter_classroom.students]
    assert (
        "Tom Riddle" not in student_names
    ), "Tom Riddle should not be in the classroom since they were never added"
